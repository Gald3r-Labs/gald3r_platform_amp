...
<h1 align="center">Gald3r for Amp Code (Sourcegraph)</h1>

<p align="center">
  The <strong>Amp Code (Sourcegraph) edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside Amp Code (Sourcegraph)</strong>. Part of the Gald3r framework
...
This is the **Amp Code (Sourcegraph) distribution** of Gald3r. The [`amp/`](./amp/) folder is a
complete, ready-to-deploy gald3r setup tuned for Amp Code (Sourcegraph) -- drop its contents into your
...
cp -r gald3r_platform_amp/amp/. /path/to/your/project/
```

Then open the project in Amp Code (Sourcegraph) and type `@g-status` to confirm gald3r loaded.

> The `amp/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
